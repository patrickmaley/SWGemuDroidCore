...
void CommandConfigManager::registerSpecialCommands(CommandList* sCommands) {
	...
	// createbot is intentionally not part of the shipped command tables.
	QueueCommand* createBot = createCommand(String("createbot").toLowerCase());
	if (createBot == nullptr) {
		error("Could not create command /createbot");
	}
